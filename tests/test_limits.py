...
from langchain_loadout import Answer, JudgeMisconfigured, Limits, Pick, Settings, Skill, SkillRouter, Turn, YesNo
from langchain_loadout.testing import ScriptedJudge, yes

TEXT = "Instructions."


def skill(name: str, group: str, description: str = "") -> Skill:
    async def read() -> str:
        return TEXT

    return Skill(name=name, description=description or f"Description of {name} " * 20, read=read, group=group)


# three groups of four skills; each option costs about (name + description) characters
CATALOG = [skill(f"{g}-{i}", g) for g in ("docs", "spend", "tax") for i in range(4)]
